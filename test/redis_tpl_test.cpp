...
int main() {
    ...
    if (auto h_val = hash_ops.hget("my_hash", "field1")) {
        std::cout << "HGET my_hash field1: " << *h_val << '\n';
    }

    // === Custom Hash Object Operations ===
    std::cout << "\n--- Custom Hash (User Object) ---\n";
    ...
    std::cout << "Saved user 'Sandro' to hash user:101\n";
    ...
    if (set_ops.sismember("my_set", "member2")) {
        std::cout << "SISMEMBER: my_set contains 'member2'\n";
    }
    ...
    std::cout << "ZRANGE my_zset (with scores):\n";
    ...
    try {
        ...
        std::cout << "Script loaded, SHA1: " << sha1 << '\n';
    }
